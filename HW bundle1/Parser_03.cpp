...
{	
 ...
 // 	//we'll also convert everything to caps like we did before:
	...
	//have user input it
	//run for loop
	// use for loop  and isspace in conjunction to space out lette3rs
	// then cout by arrary number
	//experiment with isspace(charVariable)
	//
	string tester  =  "Take the gem";
	
	
	
	for ( int x = 0; x < 4; x++)
	{
		tester.at(x) = isspace(tester.at(x));
		
		for ( int j = 0; j < tester.size(); j++){
			tester.at(j) = toupper(tester.at(j));
		//cout << tester;
		}
